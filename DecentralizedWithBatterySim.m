...
for OrderSet = 1:1:5
    fprintf("OrderSet %d has arrived, Two layer Decentralized Task Allotment has Began.\n", OrderSet);
...
title('Distance Traveled by Each Robot(Decentralized with Battery)');
grid on;
xlim([0.5, 25.5]);
ylim([0 15000]);
xticks(1:25);
...
title('Number of times charge station visited by Each Robot(Decentralized with Battery)');
grid on;
xlim([0.5, 25.5]);
ylim([0 10]);
xticks(1:25);
